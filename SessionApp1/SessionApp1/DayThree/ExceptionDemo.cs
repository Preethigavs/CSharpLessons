    ...
    internal class ExceptionDemo
    {
        ...
        public static void NestedTryDemo()
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("\tInside NestedCatch " + err.Message);  // if no throw then "Inside outer Try1 After Nested  ly", "Result" will run.
                   //throw err; // if we re throw that, only formaException  we run
                   throw new Exception("Test msg");  // if we throw that, only formaException  we run
                }
                finally
                {
                    Console.WriteLine("\tInside Nested  ly");
                }
                Console.WriteLine("Inside outer Try1 After Nested  ly"); // it wont execute when we give /0 beacuse of this try didn't catch that error.
